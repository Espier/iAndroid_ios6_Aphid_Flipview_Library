...
public class FlipCards {
	...
	private final FlipViewController controller;
	...
	private void turnPage(boolean oriVertical, float deltaNum) {
		lastPageIndex = getPageIndexFromAngle(accumulatedAngle);
		lastPosition=10f;
		float delta = deltaNum;
		orientationVertical = oriVertical;
		// orientationVertical ? (lastPosition - event.getY()) : (lastPosition -
		// event.getX());
		...
		if (state == STATE_TOUCH) {
			...
			if (Math.abs(angleDelta) > MAX_TOUCH_MOVE_ANGLE) {// prevent large
																// delta when
																// moving too
																// fast
				angleDelta = Math.signum(angleDelta) * MAX_TOUCH_MOVE_ANGLE;
			...
			if (accumulatedAngle < 0) {
				accumulatedAngle = Math.max(accumulatedAngle,
						controller.isOverFlipEnabled() ? -MAX_TIP_ANGLE : 0);
			}

			int anglePageIndex = getPageIndexFromAngle(accumulatedAngle);
			System.out.println("anglePageIndex==="+anglePageIndex+",accumulatedAngle=="+accumulatedAngle);
			System.out.println(" frontCards.getIndex()==="+ frontCards.getIndex());
			System.out.println(" backCards.getIndex()===="+backCards.getIndex());
			if (accumulatedAngle >= 0) {
				
				if (anglePageIndex != frontCards.getIndex()) {
					if (anglePageIndex == frontCards.getIndex() - 1) { // moved
																		// to
						System.out.println("turn left=====");												// previous
																		// page
						swapCards(); // frontCards becomes the backCards
						frontCards.resetWithIndex(backCards.getIndex() - 1);
						controller.flippedToView(anglePageIndex, false);
